...
export function drawDoor(
  ...
  // Laibung links/rechts (kurze Querstriche quer zur Wand) — visuelle Markierung der Öffnungs-Enden
  ...
  // Türblatt: Linie vom hinge zum leafEnd
  ...
  // Öffnungs-Bogen (Viertelkreis): vom leafEnd zurück zur gegenüberliegenden Öffnungsseite
  // Bogen-Mittelpunkt = hinge, Radius = widthM
  const opp = door.hand === "left" ? g.rightEnd : g.leftEnd;
  const sCenter = cam.worldToScreen(g.hinge.x, g.hinge.y);
  // Radius in Pixel: distance hinge → leafEnd (oder hinge → opp); identisch.
  const radiusPx = Math.hypot(sl.x - sCenter.x, sl.y - sCenter.y);
  const startAng = Math.atan2(sl.y - sCenter.y, sl.x - sCenter.x);
  const endAng = Math.atan2(cam.worldToScreen(opp.x, opp.y).y - sCenter.y,
                            cam.worldToScreen(opp.x, opp.y).x - sCenter.x);
  ctx.lineWidth = 1.2;
  ctx.beginPath();
  // Wähle die kürzere Richtung (Viertelkreis = 90°)
